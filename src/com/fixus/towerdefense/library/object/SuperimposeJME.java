...
package com.fixus.towerdefense.library.object;
...
//        ninja = assetManager.loadModel("Models/Ninja/Ninja.mesh.xml");
        ninja = assetManager.loadModel(com.fixus.towerdefense.library.configuration.Configuration.modelPath);
...
//        Material mat = new Material(assetManager, "Common/MatDefs/Misc/ShowNormals.j3md"); // default material
      Material mat = new Material(assetManager, com.fixus.towerdefense.library.configuration.Configuration.materialPath); // default material
...
//        mat.setTexture("ColorMap", assetManager.loadTexture("Textures/Ninja/Ninja.jpg"));
      mat.setTexture("ColorMap", assetManager.loadTexture(com.fixus.towerdefense.library.configuration.Configuration.texturePath));
	...
	public void animationStart(String animationName, LoopMode loopMode, float animSpeed) {
		try {
			mAniControl = ninja.getControl(AnimControl.class);
			mAniControl.addListener(this);
			mAniChannel = mAniControl.createChannel();
			mAniChannel.setAnim(animationName);
			mAniChannel.setLoopMode(loopMode);
			mAniChannel.setSpeed(animSpeed);			
		} catch(Exception ex) {
			Log.e("3D", "error with animation");
		}
	}
	
	public void animationStop() {
		if(this.mAniControl != null) {
			this.mAniControl.setEnabled(false);
		}
	}
